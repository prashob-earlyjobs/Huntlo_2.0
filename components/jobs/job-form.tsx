...
export function JobForm() {
  ...
  const currentStep = useMemo(() => {
    if (!form.title || !form.department || !form.location) return 0;
    if (form.requiredSkills.length === 0) return 1;
    if (!form.description) return 2;
    if (!form.minSalary || !form.maxSalary) return 3;
    if (!form.hiringManager) return 4;
    if (!form.screeningObjective) return 5;
    return 6;
  }, [form]);
  ...
  function persist(mode: "draft" | "publish" | "source") {
    ...
    window.setTimeout(() => {
      ...
      router.push(mode === "draft" ? ROUTES.jobs : jobDetailPath("j1"));
    }, 600);
  }
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              {saving === "source" ? "Saving…" : "Save and Source Candidates"}
            </Button>
          </div>
        }
      />
      ...
      <div className="rounded-xl border border-border bg-card p-4">
        <Stepper
          steps={JOB_FORM_SECTIONS.map((section) => ({
            id: section.id,
            title: section.title,
          }))}
      ...
      {touchedPublish && Object.keys(errors).length > 0 ? (
        ...
        >
          Fix the highlighted fields in Basic Details before publishing.
        </div>
      ) : null}
      ...
      >
        ...
        <FormSection
          title="6. Screening Configuration"
