/**
 * Outreach combo smoke test — standalone. Does NOT modify any app source files.
 *
 * Creates one pool candidate (target email/phone), then launches every
 * Single / Multi channel combination so you can verify email, WhatsApp,
 * AI Voice, and AI qualification replies end-to-end (API + worker + Redis).
 *
 * Prerequisites (already running):
 *   - API:   npm run dev          (Backend)
 *   - Worker: npm run worker
 *   - Redis:  redis-server
 *   - Connected Email / WhatsApp / Voice integrations in the workspace
 *
 * Usage (from Backend/):
 *   npx tsx scripts/outreach-combo-smoke.ts
 *
 * Edit the HARDCODED CONFIG block below before running.
 */

/* ------------------------------------------------------------------ */
/* HARDCODED CONFIG — edit these, do not use env vars                   */
/* ------------------------------------------------------------------ */
...
type Channel = 'email' | 'whatsapp' | 'ai_voice';
...
type Combo = {
  ...
  channels: Channel[];
  /** Sequence send steps (delays in minutes for smoke). */
  steps: Array<{
    type: Channel;
    delayMinutes: number;
...
function selectedCombos(): Combo[] {
  ...
  if (!picked.length) {
    throw new Error(
      `No combos matched SMOKE_COMBOS=${COMBOS_RAW}. Valid ids:\n` +
...
function channelConfig(channels: Channel[]) {
  return {
...
function sequenceSteps(combo: Combo) {
...
function qualificationConfig(enabled: boolean) {
  if (!enabled) {
    return {
      enabled: false,
      questions: [],
...
async function upsertTargetCandidate(): Promise<string> {
  // Try find by email via pool search
  type PoolRow = { id: string; email?: string | null; phone?: string | null; name?: string };
  const listed = await api<{ items: PoolRow[] } | PoolRow[]>(
    'GET',
  ...
  const created = await api<PoolRow>('POST', '/candidate-pool', {
    name: TARGET_NAME,
    email: TARGET_EMAIL || null,
    phone: TARGET_PHONE || null,
    currentTitle: 'Smoke Test Role',
...
async function runCombo(combo: Combo, candidateId: string) {
  const stamp = new Date().toISOString().replace(/[:.]/g, '-');
  const name = `[SMOKE] ${combo.label} · ${stamp}`;

  console.log(`\n→ ${combo.label}`);

  const campaign = await api<{ id: string; status?: string }>(
    'POST',
    '/outreach-campaigns',
    {
      name,
  ...
  try {
    const launched = await api<{ id?: string; status?: string }>(
      'POST',
      `/outreach-campaigns/${campaign.id}/launch`
    );
    ...
    if (err && typeof err === 'object' && 'body' in err) {
      console.error(`  details:`, JSON.stringify((err as { body: unknown }).body, null, 2));
    }
    return {
      combo: combo.id,
...
async function main() {
  console.log('═══════════════════════════════════════════════════');
  console.log(' Huntlo outreach combo smoke test (standalone)');
  console.log('═══════════════════════════════════════════════════');
  console.log(` API:     ${API}`);
  ...
  if (!LOGIN_EMAIL || !LOGIN_PASSWORD || LOGIN_PASSWORD === 'YOUR_PASSWORD_HERE') {
    throw new Error(
      'Edit HARDCODED CONFIG at the top of scripts/outreach-combo-smoke.ts — set LOGIN_EMAIL and LOGIN_PASSWORD.'
    );
  }
  ...
  console.log(` Running ${combos.length} combo(s):\n` +
