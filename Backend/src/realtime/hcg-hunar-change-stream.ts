...
import { hcgHunarOverallAiStatus } from '../modules/conversations/hcg-hunar-overlay.js';
import { OutreachCampaignModel } from '../modules/outreach/campaign.model.js';
import { emitHcgHunarUpdated } from './events.js';

const COLLECTION = 'hcg_hunar_communications';
const DEBOUNCE_MS = 400;
const ORG_CACHE_TTL_MS = 5 * 60_000;
const RESTART_DELAY_MS = 3_000;

type HunarCommunicationChange = {
...
function logger() {
  return getLogger().child({ component: 'hcg-hunar-change-stream' });
}
...
function changeReasons(
  change: HunarCommunicationChange
  ...
  debounceTimers.set(
    ...
    setTimeout(() => {
      void flushPending(docId).catch((error) => {
        logger().warn({ err: error, docId }, 'Failed to emit Hunar communication change');
      });
    }, DEBOUNCE_MS)
  );
}
...
async function openStream(): Promise<void> {
  ...
  try {
    ...
    log.warn(
      { err: error },
      'Hunar communication change stream is unavailable (replica set required)'
    );
    return;
  }

  stream.on('change', (change) => {
    queueChange(change as HunarCommunicationChange);
  });
  stream.on('error', (error) => {
    log.warn({ err: error }, 'Hunar communication change stream error; restarting');
    void closeStream().then(scheduleRestart);
  });
  stream.on('close', () => {
    if (!stopped) scheduleRestart();
  });
  log.info({ collection: COLLECTION }, 'Watching hcg_hunar_communications for live UI updates');
}
...
export function startHcgHunarChangeStream(): () => Promise<void> {
  const env = getEnv();
