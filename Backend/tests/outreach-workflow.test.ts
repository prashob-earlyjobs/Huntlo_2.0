...
describe('gmail auto-calendly prompt', () => {
  ...
  it('fills the auto-screening email prompt and keeps the conversation open', async () => {
    const { buildAutoScreeningPrompt } = await import(
      '../src/modules/outreach/prompt/index.js'
    );
    const prompt = buildAutoScreeningPrompt({
      jobText: 'Senior Node.js Developer',
      candidateName: 'Gokul Kumar',
      currentRole: 'Software Engineer',
      experience: '5 years',
    ...
    expect(prompt).toContain('Senior Node.js Developer');
    expect(prompt).toContain('Keep the conversation going');
    expect(prompt).toContain('AI screening call shortly');
    expect(prompt).not.toContain('calendly');
    expect(prompt).not.toMatch(/\{\{[a-z_]+\}\}/);
  });

  it('fills the WhatsApp auto-screening prompt with one-question-at-a-time rules', async () => {
    const { buildWhatsAppAutoScreeningPrompt } = await import(
      '../src/modules/outreach/prompt/index.js'
    );
    const prompt = buildWhatsAppAutoScreeningPrompt({
      jobText: 'Senior Node.js Developer',
      candidateName: 'Gokul Kumar',
      currentRole: 'Software Engineer',
      experience: '5 years',
...
describe('gateway conversational autoReply flags', () => {
  it('keeps autoReply on for auto-screening, calendly, and close — off only for post-qual WhatsApp', async () => {
    const {
      campaignUsesGatewayConversationalReply,
      resolveGatewayOutreachPromptKind,
    } = await import('../src/modules/outreach/gateway-auto-reply.js');

    expect(
      campaignUsesGatewayConversationalReply({
        qualificationConfig: { autoScreening: true },
      })
    ).toBe(true);
    expect(
      resolveGatewayOutreachPromptKind({
    ...
    expect(resolveGatewayOutreachPromptKind({ qualificationConfig: {} })).toBe('close');

    expect(
      campaignUsesGatewayConversationalReply({
        qualificationConfig: { autoWhatsAppAfterQualification: true },
      })
    ).toBe(false);
  });

  it('starts outreach screening from gateway qualified/shortlisted only', async () => {
    const { shouldAutoStartOutreachScreeningFromHcg } = await import(
      '../src/modules/outreach/gateway-auto-reply.js'
    );
    expect(
      shouldAutoStartOutreachScreeningFromHcg({
        autoScreening: true,
...
describe('hcg hunar overlay', () => {
  ...
  it('reads recording url from nested url aliases', async () => {
    const { hcgHunarToEvents } = await import(
      '../src/modules/conversations/hcg-hunar-overlay.js'
    );
    const event = hcgHunarToEvents({
      campaignId: 'screen-1',
      mobileNumber: '+918592929642',
      call_status: { status: 'COMPLETED', duration_seconds: 40 },
      call_recording: { url: 'https://cdn.example.com/screening.mp3' },
    })[0];
    expect(event?.voiceSummary?.recordingUrl).toContain('screening.mp3');
  });
