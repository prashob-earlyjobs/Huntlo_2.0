...
import { HERO_INTERVIEW_FLOW } from "@/lib/aiInterviewAgent";

type InterviewAgentVisualProps = {
  reduceMotion: boolean;
};

export function InterviewAgentVisual({ reduceMotion }: InterviewAgentVisualProps) {
  const steps = HERO_INTERVIEW_FLOW.slice(0, -1);
  const core = HERO_INTERVIEW_FLOW[HERO_INTERVIEW_FLOW.length - 1];
  ...
  return (
    <div
      ...
      aria-hidden
    >
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_45%_18%,_rgba(0,80,203,0.3),_transparent_55%)]" />
      <div className="absolute inset-x-10 top-1/2 h-px -translate-y-1/2 bg-gradient-to-r from-transparent via-white/15 to-transparent" />
      ...
      <div className="relative mx-auto flex max-w-md flex-col items-center">
        ...
        >
          ...
          <p className="mt-1 text-sm font-semibold text-white sm:text-base">
            Interviews as conversations, not checkpoints
          </p>
        </motion.div>
          ...
          {steps.map((step, index) => (
            <motion.span
              ...
              className={`rounded-full border px-3.5 py-2 text-[0.7rem] font-medium sm:text-xs ${
                step === "Interview Intelligence" || step === "Hiring Conversations"
