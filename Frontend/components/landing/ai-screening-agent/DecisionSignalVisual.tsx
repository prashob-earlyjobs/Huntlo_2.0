...
import { HERO_DECISION_FLOW } from "@/lib/aiScreeningAgent";

type DecisionSignalVisualProps = {
  reduceMotion: boolean;
};

export function DecisionSignalVisual({ reduceMotion }: DecisionSignalVisualProps) {
  const steps = HERO_DECISION_FLOW.slice(0, -1);
  const core = HERO_DECISION_FLOW[HERO_DECISION_FLOW.length - 1];
  ...
  return (
    <div
      ...
      aria-hidden
    >
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_18%,_rgba(0,80,203,0.32),_transparent_55%)]" />
      <div className="absolute left-1/2 top-16 h-40 w-40 -translate-x-1/2 rounded-full border border-white/10" />
      <div className="absolute left-1/2 top-24 h-24 w-24 -translate-x-1/2 rounded-full border border-[#0050cb]/30" />
          ...
          {steps.map((step, index) => (
            <motion.span
              ...
              className={`rounded-full border px-3.5 py-2 text-[0.7rem] font-medium sm:text-xs ${
                step === "Hiring Decision Intelligence"
