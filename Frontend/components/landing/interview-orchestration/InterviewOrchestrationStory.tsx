...
import {
  FUTURE_UNDERSTANDS,
  INTERVIEW_UNDERSTANDS,
  TRADITIONAL_UNDERSTANDS,
} from "@/lib/interviewOrchestration";

type InterviewOrchestrationStoryProps = {
  reduceMotion: boolean;
};

export function InterviewOrchestrationStory({ reduceMotion }: InterviewOrchestrationStoryProps) {
  return (
    <>
        ...
        <div className="mx-auto max-w-6xl">
          <div className="max-w-3xl">
            ...
            <h2 className="mt-4 text-[1.75rem] font-bold leading-tight tracking-tight text-[#141b2b] md:text-[2.5rem]">
              Great Interviews Require Great Context.
            </h2>
          ...
          <div className="mt-10 grid gap-6 lg:grid-cols-2">
            <div className="rounded-2xl border border-[#c3c6d6]/35 bg-[#f7f8fc] p-6">
              <h3 className="text-sm font-semibold uppercase tracking-[0.16em] text-[#434654]">
                Traditional recruiting understands
              </h3>
              <ol className="mt-5 space-y-2">
                {TRADITIONAL_UNDERSTANDS.map((item, index) => (
                  <li
                    key={item}
            ...
            <div className="rounded-2xl border border-[#0050cb]/25 bg-[#0050cb]/8 p-6">
              <h3 className="text-sm font-semibold uppercase tracking-[0.16em] text-[#0050cb]">
                Future hiring continuously understands
              </h3>
              <ol className="mt-5 space-y-2">
                {FUTURE_UNDERSTANDS.map((item, index) => (
        ...
        <div className="mx-auto max-w-6xl">
          <div className="max-w-3xl">
            ...
            <h2 className="mt-4 text-[1.75rem] font-bold leading-tight tracking-tight md:text-[2.5rem]">
              Introducing Interview Intelligence™
            </h2>
            <p className="mt-5 text-base leading-relaxed text-white/65 md:text-lg">
              Continuously understanding context, intent, insights, signals, confidence, priorities,
              and outcomes — before organizations ever make hiring decisions.
            </p>
          </div>

          <ul className="mt-10 space-y-3">
            {INTERVIEW_UNDERSTANDS.map((item, index) => (
              <motion.li
                key={item}
          ...
          <p className="mt-10 max-w-2xl text-base leading-relaxed text-white/70 md:text-lg">
            Future organizations won&apos;t ask which candidate performed best. They&apos;ll ask
            which hiring decision creates exceptional business outcomes.
          </p>
        </div>
      </section>
    </>
