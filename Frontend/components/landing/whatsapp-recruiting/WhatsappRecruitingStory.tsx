...
import {
  MODERN_LOOKS,
  REALTIME_UNDERSTANDS,
  TRADITIONAL_LOOKS,
} from "@/lib/whatsappRecruiting";

type WhatsappRecruitingStoryProps = {
  reduceMotion: boolean;
};

export function WhatsappRecruitingStory({ reduceMotion }: WhatsappRecruitingStoryProps) {
  return (
    <>
        ...
        <div className="mx-auto max-w-6xl">
          <div className="max-w-3xl">
            ...
            <h2 className="mt-4 text-[1.75rem] font-bold leading-tight tracking-tight text-[#141b2b] md:text-[2.5rem]">
              Hiring Momentum Happens In Minutes.
            </h2>
          ...
          <div className="mt-10 grid gap-6 lg:grid-cols-2">
            <div className="rounded-2xl border border-[#c3c6d6]/35 bg-[#f7f8fc] p-6">
              ...
              <ol className="mt-5 space-y-2">
                {TRADITIONAL_LOOKS.map((item, index) => (
                  <li
                    key={`${item}-${index}`}
            ...
            <div className="rounded-2xl border border-[#0050cb]/25 bg-[#0050cb]/8 p-6">
              <h3 className="text-sm font-semibold uppercase tracking-[0.16em] text-[#0050cb]">
                Modern hiring looks like
              </h3>
              <ol className="mt-5 space-y-2">
                {MODERN_LOOKS.map((item, index) => (
        ...
        <div className="mx-auto max-w-6xl">
          <div className="max-w-3xl">
            ...
            <h2 className="mt-4 text-[1.75rem] font-bold leading-tight tracking-tight md:text-[2.5rem]">
              Introducing Real Time Hiring Intelligence™
            </h2>
            <p className="mt-5 text-base leading-relaxed text-white/65 md:text-lg">
              Continuously understanding intent, readiness, priorities, experiences, momentum, and
              outcomes — before recruiters ever need to send another follow-up.
            </p>
          </div>

          <ul className="mt-10 space-y-3">
            {REALTIME_UNDERSTANDS.map((item, index) => (
              <motion.li
                key={item}
          ...
          <p className="mt-10 max-w-2xl text-base leading-relaxed text-white/70 md:text-lg">
            Future organizations won&apos;t ask which message they should send. They&apos;ll ask
            which conversations should happen right now.
          </p>
        </div>
      </section>
    </>
