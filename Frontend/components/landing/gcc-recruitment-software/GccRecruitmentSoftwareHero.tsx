...
import { GccCommandCenterVisual } from "./GccCommandCenterVisual";

type GccRecruitmentSoftwareHeroProps = {
  reduceMotion: boolean;
};

export function GccRecruitmentSoftwareHero({
  reduceMotion,
}: GccRecruitmentSoftwareHeroProps) {
  return (
    <section className="relative overflow-hidden bg-[#050914] text-white">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_20%_0%,_rgba(0,80,203,0.28),_transparent_50%)]" aria-hidden />
      ...
      <div className="relative mx-auto grid max-w-6xl items-center gap-12 px-4 py-20 md:px-8 md:py-28 lg:grid-cols-[1.15fr_0.85fr] lg:gap-16 lg:px-12 lg:py-32">
        ...
        >
          ...
          <h1 className="mt-5 max-w-xl text-[2.1rem] font-bold leading-[1.08] tracking-tight sm:text-[2.6rem] md:text-[3.1rem]">
            Recruiting software built for how GCCs actually hire
          </h1>
          <p className="mt-6 max-w-lg text-base leading-relaxed text-white/65 md:text-lg">
            Help your teams source candidates, run recruiter workflows, and hire across functions
            from one AI-powered platform — without juggling disconnected tools.
          </p>
        ...
        >
          <GccCommandCenterVisual reduceMotion={reduceMotion} />
        </motion.div>
      </div>
    </section>
  );
