...
import {
  CONVERSATION_UNDERSTANDS,
  FUTURE_OPTIMIZES,
  TODAY_OPTIMIZES,
} from "@/lib/outreachEngine";

type OutreachEngineStoryProps = {
  reduceMotion: boolean;
};

export function OutreachEngineStory({ reduceMotion }: OutreachEngineStoryProps) {
  return (
    <>
          ...
          <div className="mt-10 grid gap-6 lg:grid-cols-2">
            <div className="rounded-2xl border border-[#c3c6d6]/35 bg-[#f7f8fc] p-6">
              <h3 className="text-sm font-semibold uppercase tracking-[0.16em] text-[#434654]">
                Today&apos;s recruiting teams continuously optimize
              </h3>
        ...
        <div className="mx-auto max-w-6xl">
          <div className="max-w-3xl">
            ...
            <h2 className="mt-4 text-[1.75rem] font-bold leading-tight tracking-tight md:text-[2.5rem]">
              Introducing Conversation Intelligence™
            </h2>
            <p className="mt-5 text-base leading-relaxed text-white/65 md:text-lg">
              Continuously understanding conversations — before they ever begin.
            </p>
          </div>

          <ul className="mt-10 space-y-3">
            {CONVERSATION_UNDERSTANDS.map((item, index) => (
              <motion.li
                key={item}
          ...
          <p className="mt-10 max-w-2xl text-base leading-relaxed text-white/70 md:text-lg">
            Future recruiting teams won&apos;t ask which message they should send. They&apos;ll ask
            which conversation they should begin.
          </p>
        </div>
      </section>
    </>
