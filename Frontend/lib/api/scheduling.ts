...
function mapInterview(row: Record<string, unknown>): Interview {
  return {
    ...
    candidateName: String(row.candidateName || "").trim() || "Candidate",
    candidateTitle: String(row.candidateTitle || "").trim(),
    candidateCompany: String(row.candidateCompany || "").trim(),
    candidateEmail:
      String(row.candidateEmail || row.inviteeEmail || "").trim() || null,
    candidatePhone: String(row.candidatePhone || "").trim() || null,
    jobId: (row.jobId as string | null) ?? null,
    jobTitle: String(row.jobTitle || "").trim(),
    interviewType: String(row.interviewType || "").trim(),
    interviewers: (row.interviewers as string[]) || [],
    recruiter: String(row.recruiter || "").trim(),
    ...
    meetingLink: String(row.meetingLink || row.meetingUrl || "").trim() || null,
    location: String(row.location || "").trim() || null,
    bookingSource: String(row.bookingSource || "").trim(),
    reminderStatus: String(row.reminderStatus || "").trim(),
    reminderHours: Array.isArray(row.reminderHours)
      ? (row.reminderHours as number[]).filter((hours) => Number.isFinite(hours))
      : [],
