/**
 * Huntlo system-email chrome for admin preview.
 * HTML 4 / table layout + inline styles only — keep SMTP-client friendly.
 * Keep in sync with Backend/src/providers/system-mail/email-layout.ts.
 */
...
export function personalizePreviewContent(
  template: string,
  vars: { firstName?: string; email?: string } = {}
): string {
  const firstName = (vars.firstName || "Alex").trim() || "Alex";
  const email = (vars.email || "alex@example.com").trim();
...
export function buildEmailPreviewDocument(input: {
  subject: string;
  bodyHtml: string;
  firstName?: string;
  email?: string;
  ...
  return {
    subject: personalizePreviewContent(input.subject, vars),
    html: wrapHuntloEmailHtml(personalizePreviewContent(input.bodyHtml, vars)),
  };
}
